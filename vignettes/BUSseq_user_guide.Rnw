...
one.
  ...
  If we have no prior knowledge about the cell type number, we can vary
  the argument \Robject{n.celltypes} in the function 
  \Rfunction{BUSseq\_MCMC}, e.g., from 2 to 10 and identify the 
  underlying true cell type number K as the one that achieves the 
  minimal BIC. \\
  
  \noindent The user can obtain the BIC value from the 
  \Robject{BUSseqfits\_res} by the \Rfunction{BIC\_BUSseq}.\\
  
  <<bic>>=
     BIC_val <- BIC_BUSseq(BUSseqfits_res)
  @
  
      \noindent In this example, the underlying true number of cell 
      types is four. For an illustration, we vary the 
      \Robject{n.celltypes} from 3 to 6.
   <<selection>>=
   set.seed(123)
   BIC_values <- rep(NA,4)
   BUSseqfits_temp <- NULL
   for(k in 3:6){
    BUSseqfits_temp[[k-2]] <- BUSseq_MCMC(ObservedData = CountData, 
                            n.celltypes = k, n.cores = 4, 
                            n.iterations = 500, 
                            working_dir = paste0("./K",k))
    BIC_values[k-2] <- BIC_BUSseq(BUSseqfits_temp[[k-2]])
   }
   plot(3:6, BIC_values, xlab="cell type number", ylab="BIC", 
        main="BIC plot", type="b")
   @
    
    \noindent The BIC attains the minimum at \Robject{n.celltypes}=4, 
    thus correctly recovering the true cell type number.
