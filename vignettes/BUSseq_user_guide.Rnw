...
scRNA-seq data\\ User's Guide}
  ...
  to a cell. Specifically, assuming there are two batches, the R list
  consists of two read count matrices with genes in rows and cells in
  ...
  #The length of the list is two, so we have two batches
  length(CountData)
  ...
  @
    ...
    \noindent The example data \Robject{CountData} consist of two 
    batches. In total, 300 genes are measured. 
...
events.\\
  ...
  @ 
    ...
    is a list of length two, corresponding to the two batches in 
    the study. 
...
one.
  ...
  <<IG>>=
    ...
    false_discovery_ind <- !(intrinsic_gene_indices %in% 1:240)
